...
const CardInfo = ({ item }) => {
  ...
  return (
    <>
      <List mediaList noHairlinesBetween style={{ margin: 0 }}>
        <ListItem
      ...
      {!isEmpty(item.items) && (
          item.items.map((item, index) => (
            <List simple-list key={index} style={{ margin: '10px 0' }}>
              <ListItem className={styles.listItem} title="油品名称" after={item.spareName || ''} />
