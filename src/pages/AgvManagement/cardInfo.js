...
import AgvManagementApi from '~/api/AgvManagement';
...
const CardInfo = ({
  ...
  const handlePause = async (record) => {
    createDialog(
      ...
      async function () {
        await AgvManagementApi.suspendTask(record.id)
          .then(res => {
  ...
  const handleComplete = (record) => {
    createDialog(
      ...
      `任务单号${record.taskCode}`,
      async function () {
        await AgvManagementApi.finishTask(record.id)
          .then(res => {
  ...
  const handleContinue = (record) => {
    createDialog(
      ...
      `任务单号${record.taskCode}`,
      async function () {
        await AgvManagementApi.continueTask(record.id)
          .then(res => {
  ...
  const handleRollback = (record) => {
    createDialog(
      ...
      `任务单号${record.taskCode}`,
      async function () {
        await AgvManagementApi.backTask(record.id)
          .then(res => {
