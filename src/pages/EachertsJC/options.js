...
export const getLeftSecondecharts = (value, type) => {
  if (type.length == 0 || type == null) {
    const listnumber = moment(new Date()).format('dddd');
    //////console.log(listnumber, 'oooooo');
    type = [listnumber];
    ...
    series: [
      {
        ...
        areaStyle: {
          ...
          //       {
          //         offset: 0,
          //         color: 'rgba(61,175,237,0.3)'
          //       },
          //       {
          //         offset: 1,
          //         color: 'rgba(61,175,237,0)'
          //       }
          //     ],
          //     false
          //   ),
          //   shadowColor: 'rgba(61,175,237, 0.9)',
          //   shadowBlur: 20
          // }
        },
        data: value
...
export const getMiddleFirstechartsb = (value, value2, type) => {
  const seriesLabel = {
    show: true,
    color: '#fff', //文字的颜色
    position: 'top', //文字的位置
  ...
  const option = {
    ...
    series: [
      ...
      //     focus: 'series'
      //   },
      //   data: data.consumptionOreBlending
      // }
    ]
  };
  ...
  //     type: 'category',
  //     inverse: true,
  //     data: productInOutStorageData.groups,
  //     axisLabel: {
  //       margin: 20,
  //       fontSize: 2.4 * window.rem + 'px',
  //       color: '#fff',
  ...
  //       name: '入库',
  //       type: 'bar',
  //       data: productInOutStorageData.inStock,
  //       label: seriesLabel,
  //       itemStyle: {
  //         //图形的样式
  ...
  //       type: 'bar',
  //       label: seriesLabel,
  //       data: productInOutStorageData.outStock,
  //       itemStyle: {
  //         borderRadius: [20, 20, 20, 20],
  //         color: {
  //           type: 'linear',
...
export const getMiddleSecendecharts = (value, value2, type) => {
  function formatNumber(text) {
    // 检查text是否为null或者undefined
    if (text === null || text === undefined) {
      return null; // 如果是，返回一个错误信息
    }
    ...
    if (!isNaN(text) && text.trim() !== '') {
      ...
      // Not a number, return the original text
      return text;
    }
  }
  const sumvalue = value;
  const sumvalue2 = value2;
  if (value == null) {
    value = [];
  }
  if (value2 == null) {
  ...
  if (value.length > 0) {
    value = sumvalue.map(item => {
      //////////console.log(item);
      return formatNumber(item);
    });
  }
  if (value2.length > 0) {
    value2 = sumvalue2.map(item => {
      //////////console.log(item);
      return formatNumber(item);
    });
  }
...
export const getRightFirstechartsb = (value, value2, value3, type) => {
  function formatNumber(text) {
    // 检查text是否为null或者undefined
    if (text === null || text === undefined) {
      return null; // 如果是，返回一个错误信息
  ...
  for (let i = 0; i <= value.length; i++) {
    if (value[i] != null && value2[i] != null) {
      sumalldata[i] = (Number(value[i]) + Number(value2[i])).toFixed(1);
    } else {
      sumalldata[i] = null;
    }
  }
  ...
  if (type != null) {
    if (type.length > 12) {
      ...
      if (listnumber < 25) {
        listoverstart = lastmouth - 25 + listnumber - 6;
        listoverend = lastmouth - 25 + listnumber;
        console.log(listoverstart, listoverend);
      }
      if (listnumber >= 25) {
        listoverstart = 0;
        listoverend = 6;
...
export const getRightThirdechartsb = (value, type) => {
  const seriesLabel = {
    show: true,
    color: '#fff', //文字的颜色
    position: 'right', //文字的位置
