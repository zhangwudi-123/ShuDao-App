...
const appName = 'http://47.108.91.44:9032/shudaogroup_ydong/Largescreen';
const appName2 = 'http://47.108.91.44:9032/shudaogroup_ydong/SyncToGroup';
const appName3 = 'http://175.153.115.145:9000/ledger-services/Largescreen';
class Minescreen2 extends Service {
  //营收饼状图
  ...
  async getProductPrice(data, name) {
    return await this.get(`${appName}/getProductPrice/${data}/${name}`);
  }
  //库存名字
  async getProductPriceName(data) {
    return await this.get(`${appName}/getProductPriceName/${data}`);
  }
  //库存详情柱状图月
  ...
  //获取营收折线图
  async getRevenueMonthDay() {
    try {
      return await this.get(`${appName3}/getRevenueMonthDay`);
    } catch (error) {
      throw new Error(error);
    }
  }
  //成本图表
  async getFactoryCostAuto() {
    try {
      return await this.get(`${appName3}/getFactoryCostAuto`);
    } catch (error) {
      throw new Error(error);
    }
  }
...
export default new Minescreen2();
